...
public class Bad_Boss_Manager : MonoBehaviour
{
    ...
    IEnumerator Random_Movement()
    {
        while (true)
        {
            ...
            Vector3 myDirection = Target.transform.position - transform.position; // Finding the Direction of target relative to origin of bullet (Boss's position)
            myRB.AddForce(myDirection * speed);
            //Debug.Log(myDirection.y);
            yield return new WaitForSeconds(1.0f); //Interval between bullets, current set at 3 seconds
        }
    }
